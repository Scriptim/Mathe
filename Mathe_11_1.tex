...
\end{gather*}
*Diskriminante:\\
$ > 0 \quad\Rightarrow\quad \text{zwei Lösungen} $ \\
$ = 0 \quad\Rightarrow\quad \text{eine Lösung} $ \\
$ < 0 \quad\Rightarrow\quad \text{keine Lösung} $
\begin{gather*}
  ...
  \Rightarrow x_1 + x_2 = -p \quad x_1 \cdot x_2 = q \\\\
...
\begin{gather*}
  ...
  \quad x_1 = \sqrt{2.5}; \; x_2 = -\sqrt{2.5} \\
  t_2 = x^2 = -1 \equ \sqrt{} \\
  \quad \text{keine Lösung} \\
  L = \{\sqrt{2.5}; -\sqrt{2.5}\}
\end{gather*}
...
\begin{gather*}
  a_n = (\frac{1}{2})^n \\
  a_1 = \frac{1}{2};\quad a_2 = \frac{1}{4};\quad a_3 = \frac{1}{8}; \quad\dots;\quad a_{100} = 2^{-100}
\end{gather*}
...
\begin{exercise}{16/2}
  ...
  \item [d]
  \begin{gather*}
    a_1 = 0;\; a_2 = 1;\; a_{n + 2} = a_n + a_{n + 1}\quad \text{(Fibonacci)} \\
...
$\lim\limits_{n \to \infty} a_n = g\qquad \text{hier:}\; \lim\limits_{n \to \infty} n^{-1} = 0$
\newpage
\begin{exercise}{22/2}
  ...
  \begin{gather*}
    ...
    \epsilon > \sqrt{10} \approx 3.162\quad \text{(ab 4)}
  \end{gather*}
  \item [c]
  \begin{gather*}
    a_n = 1 - \frac{100}{n} \\
  ...
  \begin{gather*}
    ...
    n < \sqrt{-\frac{30}{7}} \quad\text{und}\quad n < -\sqrt{-\frac{30}{7}} \\
...
$a_n = \frac{n}{n+1}\quad g = 1\quad \rightarrow\quad a_n^\ast = \frac{n}{n+1} - 1\quad g = 0\quad \text{(Nullfolge)}$
\begin{exercise}{22/4}
  \item [a]
  \begin{gather*}
...
$f(x) = y = \frac{3x^2 - 3}{(x + 1)(x - 4)} \qquad \text{D} = \mathbb{R}\setminus\{\underbrace{-1; 4}_{\mathclap{\text{Nullstellen des Nenners}}}\}$ \\\\
...
\end{tabular}
\begin{gather*}
  \lim\limits_{ x \nearrow 4} f(x) = -\infty \qquad \lim\limits_{ x \searrow 4} f(x) = +\infty
\end{gather*}
\begin{tikzpicture}
  \begin{axis}
    \addplot[
...
\begin{gather*}
  ...
  n = 4 \quad\text{(Grad } n \in \mathbb{N}\text{)} \\ a_4 = 4 \\ a_3 = a_2 = 0 \\ a_1 = 5 \\ a_0 = -7 \\
...
\begin{exercise}{28/11}
  \item
  \begin{gather*}
    f(x) = sin(\frac{1}{x}) \qquad D_f = \mathbb{R} \setminus \{0\} \\
    \lim\limits_{x \to 0} f(x) = \sin(\infty) = -1 \text{ bis } +1 \quad\Rightarrow \text{kein Grenzwert} \\
...
\begin{exercise}{29/4}
  \item [a]
  $a_n = \frac{n^2 - 7n - 1}{10n^2 - 7n}\quad \lim\limits_{n \to \infty} a_n = \frac{1}{10}$
  \item [b]
  $a_n = \frac{n^3 - 3n^2 + 3n - 1}{5n^3 - 8n + 5}\quad \lim\limits_{n \to \infty} a_n = \frac{1}{5}$
  \item [f]
  $a_n = \frac{2^{n + 1}}{2^n + 1}\quad \lim\limits_{n \to \infty} a_n = 2$
  \item [g]
  $a_n = \frac{3^n + 1}{5^n}\quad \lim\limits_{n \to \infty} a_n = 0$
\end{exercise}
\begin{exercise}{29/5}
  \item [a]
...
\begin{exercise}{30/11}
  \item [a]
  \begin{gather*}
    f(x) = \frac{x^2 - 2x + 1}{x - 1} = \frac{(x - 1)^2}{x - 1} = x - 1 \qquad D = \mathbb{R} \setminus \{1\} \\
    \lim\limits_{x \to 1} f(x) = x - 1 = 0
  \end{gather*}
  \item [c]
  \begin{gather*}
    f(x) = \frac{x^4 - 1}{x^2 - 1} = x^1 + 1 \qquad D = \mathbb{R} \setminus \{1; -1\} \\
...
\begin{exercise}{29/6}
  ...
  $a_5 = 0.95^5 = 0.77$
  \item [b]
  $0.5 = 0.95^n \Rightarrow n = \frac{\log(0.5)}{\log(0.95)} \approx 13.5 \quad n = 13$
  \item [c]
  \begin{gather*}
...
\begin{gather*}
  P(1|3)\;f(1) = 3 \\
  Q(10|8)\;f(10) = 8 \\
  x_1 = 1;\;x_2 = 10\quad \Delta x = x_2 - x_1 = h = 9 \\
...
\begin{gather*}
  f(x) = y = x^2\quad x_0 = 2 \\
  m = \frac{f(x_0 + h) - f(x_0)}{h} = \frac{(2 + h)^2 - 2^2}{h} = 4 + h \quad\text{(Sekante)} \\
  m = \lim\limits_{h \to 0} (4 + h) = 4 \quad\text{(Tangente)}
\end{gather*}
...
$f(x) = 5x^3\qquad P_1(2|f(2))\quad P_2(4|f(4))$ \\\\
Sekantengleichung
\begin{gather*}
  y = m \cdot x + b \\
...
\begin{exercise}{49/14}
  ...
  \item [c]
  \begin{gather*}
    f(x) = \sqrt{x}\qquad P(0.5|f(0.5) = \sqrt{0.5}) \\
...
\begin{gather*}
  g(x) = x^2 \\
  \quad g'(x) = \lim\limits_{h \to 0} \frac{(x + h)^2 - x^2}{h} = \lim\limits_{h \to 0} \frac{x^2 + xh + h^2 - x^2}{h} = 2x \\
  h(x) = x^3 \\
  \quad h'(x) = \lim\limits_{h \to 0} \frac{(x + h)^3 - x^3}{h} = \lim\limits_{h \to 0} \frac{x^3 + 3x^2h + 3xh^2 + h^3 - x^3}{h} = 3x^2 \\
  i(x) = a \cdot x^2 \\
  \quad i'(x) = \lim\limits_{h \to 0} \frac{a(x + h)^2 - a \cdot x^2}{h} = \lim\limits_{h \to 0} \frac{ax^2 + 2xh + ah^2 - ax^2}{h} = 2ax \\
  j(x) = \sqrt{x} \\
  \quad j'(x) = \lim\limits_{h \to 0} \frac{\sqrt{x + h} - \sqrt{x}}{h} = \lim\limits_{h \to 0} \frac{(\sqrt{x + h} - \sqrt{x})(\sqrt{x + h} + \sqrt{x})}{h(\sqrt{x + h} + \sqrt{x})} = \frac{1}{2\sqrt{x}} \\
  k(x) = \frac{1}{x^2} \\
  \quad k'(x) = \lim\limits_{h \to 0} \frac{(x + h)^2 - x^{-2}}{h} = \lim\limits_{h \to 0} \frac{(x^2 + 2xh + h^2)^{-1} - x^{-1}}{h} = -2x^-3
\end{gather*} \\\\
$$f(x) = ax^n \qquad f'(x) = n \cdot ax^{n-1}$$ \\\\
\newpage
\begin{exercise}{54/2}
  \item [a]
  \begin{gather*}
...
\begin{exercise}{59/6}
  ...
  \begin{gather*}
    ...
    -3 = 3bx^2 \\
    x = \pm (-b)^{-\frac{1}{2}} \\
    P(\pm (-b)^{-\frac{1}{2}} | f(\pm (-b)^{-\frac{1}{2}}) = \pm (-b)^{-\frac{1}{2}}+ c) \qquad b < 0
  \end{gather*}
\end{exercise}
...
\begin{gather*}
  \text{zu} \colon x_1 = 0 \qquad x_l = -1 \quad x_r = 1 \\
  f'(x_l) = f'(-1) = 7 \\
  f'(x_1) = f'(0) = 0 \\
  f'(x_r) = f'(1) = -1 \\
  \Rightarrow H(0|0) \\\\
  \text{zu} \colon x_2 = \frac{4}{3} \qquad x_l = 1 \quad x_r = 2 \\
  f'(x_l) = f'(1) = -1 \\
  f'(x_1) = f'(0) = 0 \\
  f'(x_r) = f'(2) = 4 \\
...
\begin{gather*}
  f'(x_l) < 0 \quad f'(x) = 0 \quad f'(x_r) < 0 \quad \text{oder} \\
  f'(x_l) > 0 \quad f'(x) = 0 \quad f'(x_r) > 0
\end{gather*}
\begin{exercise}{73/2}
  \item [e]
  \begin{gather*}
    ...
    3 = x \\
    L = \{0; 3\} \\\\
    f'(-1) = 4 \quad f'(0) = 0 \quad f'(1) = 2 \quad \Rightarrow \text{Sattelpunkt } S(0|f(0) = -4) \\
    f'(2) = 4 \quad f'(3) = 0 \quad f'(4) = -16 \quad \Rightarrow \text{Hochpunkt } H(3|f(3) = 2\frac{3}{4})
  \end{gather*}
\end{exercise}
\newpage
\begin{exercise}{74/6}
  ...
  \begin{gather*}
    ...
    x_2 = 1 \quad x_3 = 2 \\\\
    f'(-1) = -24 \quad f'(x_1) = 0 \quad f'(\frac{1}{2}) = 1.5 \quad \Rightarrow T(0|0) \\
    f'(\frac{1}{2}) = 1.5 \quad f'(x_2) = 0 \quad f'(\frac{3}{2}) = -1.5 \quad \Rightarrow H(1|1) \\
    f'(\frac{3}{2}) = -1.5 \quad f'(x_3) = 0 \quad f'(3) = 24 \quad \Rightarrow T(2|0) \\
  \end{gather*}
  \begin{gather*}
    \begin{tikzpicture}
      \begin{axis}[
...
\begin{exercise}{80/1}
  ...
  \begin{gather*}
    ...
    f'(x) = 2 - 6x \\
    f''(x) = -6 \\
    f'(x) = 0 \quad L = \{\frac{1}{3}\} \\
    f''(\frac{1}{3}) = -6 < 0 \quad H(\frac{1}{3}|\frac{1}{3})
  \end{gather*}
  \item [d]
  \begin{gather*}
    f(x) = x^4 - 4x^2 + 3 \\
    f'(x) = 2 - 6x \\
    f''(x) = 12x^2 - 8 \\
    f'(x) = 0 \quad L = \{\pm \sqrt{2}; 0\} \\
    f''(\pm \sqrt{2}) = 16 > 0 \quad T(\pm \sqrt{2}|-1) \\
    f''(0) = -8 < 0 \quad H(0|3)
  \end{gather*}
  \item [e]
  \begin{gather*}
    ...
    f''(x) = 16x^3 - 20x \\
    f'(x) = 0 \quad L = \{\pm \frac{1}{2}; \pm \frac{3}{2}\} \\
    f''(\frac{1}{2}) = -8 < 0 \quad H(\frac{1}{2}|\frac{11}{15}) \\
    f''(-\frac{1}{2}) = 8 > 0 \quad H(-\frac{1}{2}|-\frac{11}{15}) \\
    f''(\frac{3}{2}) = 24 > 0 \quad T(\frac{3}{2}|-\frac{9}{5}) \\
    f''(-\frac{3}{2}) = -24 < 0 \quad T(-\frac{3}{2}|\frac{9}{5})
  \end{gather*}
\end{exercise}
\section{Wendepunkte}
...
\begin{exercise}{85/14}
  ...
  \begin{gather*}
    f_a(x) = x^3 - ax^2 \\
    f_a''(x) = 6x - 2a = 0 \quad L = \{\frac{a}{3}\} \\
    f_a'''(x) = 6 \quad f_a'''(\frac{a}{3}) = 6 \Rightarrow RLWP(\frac{a}{3}|f_a(\frac{a}{3}))
  \end{gather*}
  \item [b]
  \begin{gather*}
    f_a(x) = x^4 - 2ax^2 + 1 \\
    f_a''(x) = 12x^2 - 4a = 0 \quad L = \{\pm \sqrt{\frac{a}{3}}\} \\
    f_a'''(x) = 24x \\
...
\begin{exercise}{89/1}
  ...
  \begin{gather*}
    ...
    f(x) = 0 \quad L = \{0; \pm \sqrt{18}\} \Rightarrow S(0|0), S(\sqrt{18}|0), S(-\sqrt{18}|0) \\
    f'(x) = -\frac{2}{9}x^3 + 2x = 0 \quad L = \{0; \pm 3\} \\
    f''(x) = -\frac{2}{3}x^2 + 2 \\
  ...
  \begin{gather*}
    ...
    f(0) = 0 \Rightarrow S(0|0) \\
    f(x) = \frac{1}{6}x^3 - x^2 + 1.5x = 0 \quad L = \{0; 3\} \Rightarrow S(0|0), S(3|0) \\
    f'(x) = \frac{1}{2}x^2 - 2x + 1.5 = 0 \quad L = \{1; 3\} \\
    f''(x) = x - 2 \\
  ...
  \item [f]
  \begin{gather*}
    f(x) = x + \frac{5}{x} \quad D = \mathbb{R}\setminus\{0\} \quad \text{(kein Schnittpunkt mit y-Achse)} \\
    f(x) = 0 \quad L = \{\} \quad \text{(kein Schnittpunkt mit x-Achse)} \\
    f'(x) = 1 - \frac{5}{x^2} = 0 \quad L = \{\pm \sqrt{5}\} \\
    f''(x) = \frac{10}{x^3} \\
...
\begin{exercise}{98/11}
  $f(x) = x^3 - 2x^2 - 3x + 10 \qquad S(-2|0) \Rightarrow x_1 = -2$
  \item [a]
  ...
  \item [b]
  \begin{gather*}
    g(x) = mx + b \quad m = 2 \quad S(-2|0) \\
    0 = 2 \cdot (-2) + b \Rightarrow b = 4 \\
    \Rightarrow g(x) = 2x + 4 \\\\
    f(x) = g(x)
    x^3 - 2x^2 - 3x + 10 = 2x + 4 \\
    0 = x^3 - 2x^2 - 5x + 6 \quad x_1 = -2
  \end{gather*}
...
\begin{exercise}{98/12}
  ...
  \begin{gather*}
    ...
    0 = 2x^2 - 2x + 8 \\
    x_{2/3} = \frac{2 \pm \sqrt{-60}}{4} \quad \text{keine Lösung} \\
    L = \{0\} \\\\
...
\begin{exercise}{105/1}
  ...
  \begin{gather*}
    ...
    L = \{\frac{4}{3}; 4\} \\
    f''(x_1) = f''(\frac{4}{3}) = -4 < 0 \quad HP(\frac{4}{3}|f(\frac{4}{3})) \\
    f''(x_2) = f''(4) = 4 > 0 \quad TP(4|f(4)) \\
    \text{Wendestellen} \\
    f''(x) = 0 = 3x - 8 \\
    L = \{\frac{8}{3}\} \\
    f'''(x_1) = f'''(\frac{8}{3}) = 3 > 0 \quad RLWP(\frac{8}{3}|f(\frac{8}{3}))
  \end{gather*}
  \begin{gather*}
    \begin{tikzpicture}
      \begin{axis}[
  ...
  \begin{gather*}
    ...
    3x + 12 = 0 \\
    L = \{0; -4\} \\
    f''(x_1) = f''(0) = 12 > 0 \quad TP(0|f(0)) \\
    f''(x_2) = f''(-4) = -12 < 0 \quad HP(-4|f(-4)) \\
    \text{Wendestellen} \\
    f''(x) = 0 = 6x + 12 \\
    L = \{-2\} \\
    f'''(x_1) = f'''(-2) = 6 > 0 \quad RLWP(-2|f(-2))
  \end{gather*}
  \begin{gather*}
    \begin{tikzpicture}
      \begin{axis}[
...
\begin{gather*}
  f(x) = y = m \cdot x + b \\
  \text{Stelle } u \qquad f'(u) = m \qquad f(u) = y \\
  f(u) = f'(u) \cdot u + b \quad\Rightarrow\quad b = f(u) - f'(u) \cdot u \\
...
\begin{exercise}{108/5}
  ...
  \item [b]
  \begin{gather*}
    f(x) = \frac{4}{x} + 2; \quad B(4|3) \quad u = 4 \\
    f'(x) = -4x^{-2} \quad f'(u) = -\frac{1}{4} \\
...
\begin{exercise}{109/10}
  \begin{gather*}
    ...
    0 = \frac{1}{2}x_0^2 - 2 \qquad x_0 = \pm 2
  \end{gather*}
\end{exercise}
\begin{exercise}{109/11}
  \begin{gather*}
    ...
    t(0) = -1.8 = 0.006u^4 - 0.122u^3 - 1.8 \qquad L = \{0; \pm\sqrt{\frac{61}{3}}\} \\
    t(x) = 0.366752x - 1.8 \quad fuer \; u = \sqrt{\frac{61}{3}} \\
    1.6 = 0.366752x_0 - 1.8 \\
    x_0 = 9.2706
  \end{gather*}
\end{exercise}
...
\begin{exercise}{112/7}
  ...
  \begin{gather*}
    S''(t) = -0.48t + 7 \\
    S''(t_0) = 0 \\
    t_0 = 14.58\overline{3} \quad (um \; 1994)
  \end{gather*}
  \item [c]
  \begin{gather*}
    S'(t_0) = 0 = -0.24t^2 + 7t + 10.6 \\
    t_0 = \frac{1}{12}(175 + \sqrt{36985}) \approx 30.6 \quad (um \; 2010) \\
    S''(t_0) \approx -7.69 < 0 \quad\Rightarrow\quad Hochpunkt
  \end{gather*}
  \item [d]
...
\begin{exercise}{111/3}
  ...
  \item [a]
  \begin{gather*}
    f(t) = 0 = 0.25t^3 - 12t^2 + 144t \qquad L = \{0; 24\} \\\\
    f'(t) = 0 = 0.75t^2 - 24t + 144 \qquad L = \{8; 24\} \\
    f''(t) = 1.5t - 24 \\
    f''(8) = -12 < 0 \quad\Rightarrow\quad HP \\
    f''(24) = 12 > 0 \quad\Rightarrow\quad TP \\\\
    f''(t) = 0 = 1.5t - 24 \qquad L = \{16\} \\
    f'''(t) = 1.5 > 0 \quad\Rightarrow\quad RLWP
  \end{gather*}
  \item [b]
  \begin{gather*}
